...
- **Chart READMEs.** Both Helm charts now ship a `README.md` that renders on
  the ArtifactHub package card, with the CRDs-first install flow.

### Fixed

- **Chart alert-pack completeness and a stale logo reference.** The operator
  chart's `PrometheusRule` now carries the `ValkeySentinelTopologyMismatch`
  alert and the corrected sustained-split-brain rule
  (`valkey_split_brain_sustained_seconds`); the dead chart `icon` URL was
  removed.

### Security

- **Bumped vulnerable transitive dependencies.** `google.golang.org/grpc`
  1.72.2 → 1.79.3 (server authorization-bypass via a `:path` header missing the
  leading slash, GHSA / grpc-go #8981) and the OpenTelemetry Go core modules
  (`otel`, `otel/metric`, `otel/sdk`, `otel/trace`) 1.40.0 → 1.43.0
  (`baggage` multi-value DoS amplification GHSA-mh2q-q3fh-2475; SDK `kenv`
  PATH-hijacking GHSA-hfvc-g4fc-pqhx). Transitive-only bumps; no operator
  behavior change.
- **Bumped the Go toolchain floor `1.26.4` → `1.26.5`** to clear the
  standard-library advisory GO-2026-5856 (Encrypted Client Hello privacy leak
  in `crypto/tls`). Toolchain-only; no operator behavior change.
...
- **Sentinel topology hygiene surface.** Sentinel mode surfaces a sustained
...
- **Stranded-sentinel repair read-back and backoff.** The stranded-sentinel
  surgery verifies its own convergence: repeated no-progress surgeries on
...
- **Stale-epoch sentinel re-point.** During an armed re-point pass the operator
  also force-converges a sentinel monitoring a live-but-superseded primary
...
- **Recovery election under sustained quorum loss.** The zero-master recovery
  election can arm during sustained quorum-loss suppression via a paced
...
- **Dual-master detection in replication mode.** The no-labeled-primary
  observation scan stamps the same `DualMasterObserved`
...
- **Quorum-suppression entry requires fresh evidence.** The suppression gate's
  entry threshold-cross requires a fresh sentinel poll stamp, so reconcile
...
- **`InitialSentinelReset` narrowed to operator startup.** The redundant
  bootstrap-completion RESET pass was removed; the event fires only on the
...
- **Stale per-CR alert gauges after a missed delete.** The background state
  pruner clears the per-CR gauges (`valkey_dual_master_observed`,
...
- **Sentinel observer teardown on CR delete.** Deleting a sentinel-mode CR
